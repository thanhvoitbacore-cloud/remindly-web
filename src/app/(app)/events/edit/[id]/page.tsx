...
export default async function EditEventPage(props: { params: Promise<{ id: string }> }) {
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-space-6 animate-in fade-in duration-500 pb-space-12">
            <header className="flex items-center justify-between pb-space-4 border-b border-border-subtle">
                <div>
                    <h1 className="h1-premium text-text-main mb-space-2">
                        {event.isDraft ? "Resume Editing Draft" : "Sửa Sự kiện"}
                    </h1>
                    <p className="body-premium text-text-muted">
                        {event.isDraft
